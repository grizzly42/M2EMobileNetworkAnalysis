...
## function of dividing ping into appropriate categories
def pingCat(value):
    if value <= 30:
        return "≤ 30 ms"
    elif value <= 50:
...
all_days = sorted(df_all['datetime'].dt.date.unique())
...
for day in all_days:
    ...
    for i, op in enumerate(operators):
        ...
        df_day = df_op[df_op['datetime'].dt.date == day]
        axes[i].plot(df_day['datetime'], df_day['Level'], color=colors[op], linewidth=1)
    ...
    for op in operators:
        df_op = df_all[df_all["Operator"] == op]
        df_day = df_op[df_op['datetime'].dt.date == day]
        ax_all.plot(df_day['datetime'], df_day['Level'], label=op, color=colors[op], linewidth=1)
...
df_all["PING_CAT"] = df_all["PINGAVG"].apply(pingCat)
# set legend from low PING to high PING
ordered_categories = ["≤ 30 ms", "30–50 ms", "50–100 ms", "100–250 ms", "250–500 ms", "> 500 ms"]
df_all["PING_CAT"] = pd.Categorical(df_all["PING_CAT"],categories=ordered_categories, ordered=True)
...
for op in operators:
    ...
    df_op = df_all[df_all["Operator"] == op].copy()
    df_op["ping_id"] = df_op["PING_CAT"].cat.codes
    ...
    # save figure in pdf format
    plt.savefig(f"{SaveFigLoc}/PING_{op}.pdf",format="pdf",bbox_inches="tight")
plt.show()
